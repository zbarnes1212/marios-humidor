...
const T = {
  bg:"#120a02", card:"#1c1006", cardMid:"#221408",
...
function CubanRibbon({height=3}:{height?:number}) {
  return <div style={{display:"flex",height,overflow:"hidden",opacity:0.9}}>
    {[{c:"#002868",f:1.2},{c:"rgba(255,255,255,0.3)",f:0.06},{c:"#f0ece0",f:0.4},
      {c:"rgba(255,255,255,0.3)",f:0.06},{c:"#bf0a30",f:1.4},{c:"rgba(255,255,255,0.3)",f:0.06},
      {c:"#f0ece0",f:0.4},{c:"rgba(255,255,255,0.3)",f:0.06},{c:"#002868",f:1.2}
...
function CedarBg() {
  return <div style={{position:"fixed",inset:0,zIndex:0,
    background:"linear-gradient(160deg,#1c1006 0%,#120a02 55%,#0c0802 100%)",
    backgroundImage:`repeating-linear-gradient(92deg,transparent 0,transparent 26px,rgba(0,0,0,0.03) 26px,rgba(0,0,0,0.03) 27px)`}}/>;
}
  ...
  return <div style={{display:"flex",flexDirection:"column",alignItems:"center"}}>
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{filter:"drop-shadow(0 4px 18px rgba(0,0,0,0.55))"}}>
      <defs>
        ...
        <linearGradient id={`${uid}n`} x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="#8B0000"/><stop offset="50%" stopColor="#cc0020"/><stop offset="100%" stopColor="#8B0000"/></linearGradient>
...
// DATA
const HUMIDORS=[
  {id:1,name:"Govee M",wood:"Spanish Cedar",temp:68,humidity:69,capacity:150,count:87,status:"optimal"},
  {id:2,name:"Govee B",wood:"Mahogany",temp:70,humidity:67,capacity:20,count:12,status:"good"},
  {id:3,name:"Raching MON1800A",wood:"Electronic",temp:67,humidity:70,capacity:900,count:210,status:"optimal"},
];
const CIGARS=[
  {id:1,brand:"Arturo Fuente",line:"Opus X",vitola:"Robusto",origin:"Dominican Republic",wrapper:"Natural",rating:96,count:8,purchaseDate:"Jun 2023",bandColor:"#5a0808"},
  {id:2,brand:"Padrón",line:"1964 Anniversary",vitola:"No. 6",origin:"Nicaragua",wrapper:"Natural",rating:98,count:3,purchaseDate:"Nov 2022",bandColor:"#12184a"},
  {id:3,brand:"Cohiba",line:"Behike",vitola:"BHK 54",origin:"Cuba",wrapper:"Cuban Wrapper",rating:95,count:5,purchaseDate:"Aug 2021",bandColor:"#081a08"},
  {id:4,brand:"My Father",line:"Le Bijou 1922",vitola:"Torpedo",origin:"Nicaragua",wrapper:"Oscuro",rating:94,count:12,purchaseDate:"Jan 2024",bandColor:"#2e1002"},
  {id:5,brand:"Liga Privada",line:"No. 9",vitola:"Robusto",origin:"Nicaragua",wrapper:"Maduro",rating:93,count:7,purchaseDate:"Mar 2024",bandColor:"#101018"},
];
const NOTES_INIT=[
...
function HumidorsTab() {
  ...
  const fetchLive=useCallback(async()=>{
    ...
    try{
      ...
      if(data.ok&&data.sensors&&data.sensors.length>0){
        const mapped:Record<string,LiveReading>={};
        data.sensors.forEach((s:{name:string;temperature:number|null;humidity:number|null;observedAt:string|null})=>{
          mapped[s.name]={temperature:s.temperature,humidity:s.humidity,observedAt:s.observedAt};
        });
        setLiveData(mapped);
        setStatus("connected");
  ...
  const HumidorCard=({h,isHero}:{h:Humidor,isHero:boolean})=>{
    ...
    const ok=h.status==="optimal",good=h.status==="good";
    const statusColor=ok?"#5ab07a":good?T.goldMid:"#c05050";
    const statusLabel=ok?"Optimal":good?"Good":"Warning";
    return (
      <div style={{background:"linear-gradient(170deg,#2c1a08 0%,#1e0e04 50%,#180c02 100%)",
        borderRadius:20,border:`1px solid rgba(160,120,40,0.30)`,overflow:"hidden",
        boxShadow:"0 12px 40px rgba(0,0,0,0.6), inset 0 1px 0 rgba(200,160,60,0.10)"}}>
        <div style={{height:isHero?4:3,background:isHero
          ?`linear-gradient(90deg,${T.goldDark},${T.goldLight} 45%,${T.goldMid} 70%,${T.goldDark})`
          :`linear-gradient(90deg,${statusColor}88,${statusColor}44,transparent)`}}/>
        <div style={{padding:isHero?"22px 22px 0":"18px 22px 0"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:2}}>
            <div>
              <div style={{fontSize:isHero?24:20,fontWeight:"bold",color:T.textPrimary,
                fontFamily:"Georgia,serif",letterSpacing:0.2,lineHeight:1.1}}>{h.name}</div>
              <div style={{fontSize:11,color:T.textSecondary,marginTop:5,fontFamily:"Georgia,serif",letterSpacing:0.3}}>
                {h.wood}
            ...
            <div style={{background:`${statusColor}14`,border:`1px solid ${statusColor}44`,
              borderRadius:20,padding:"5px 14px",fontSize:9,color:statusColor,
              letterSpacing:2,textTransform:"uppercase",fontFamily:"Georgia,serif",marginTop:3,
            }}>{statusLabel}</div>
          </div>
          <div style={{display:"flex",justifyContent:"space-around",alignItems:"flex-end",
            gap:8,paddingBottom:4,marginTop:isHero?18:16}}>
            <LuxuryGauge value={humidity} size={136} label="%" subtitle="Humidity"/>
            <div style={{paddingBottom:isHero?48:38,width:1,
              background:`linear-gradient(180deg,transparent,${T.goldDark}28,transparent)`}}/>
            <LuxuryGauge value={temp} size={136} label="°F" subtitle="Temperature"/>
          </div>
          <div style={{marginTop:isHero?18:14,marginBottom:isHero?20:18}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <div style={{fontSize:9,color:T.textMuted,letterSpacing:2,textTransform:"uppercase",fontFamily:"Georgia,serif"}}>Fill Level</div>
              <div style={{fontSize:12,color:T.goldMid,fontFamily:"Georgia,serif",fontWeight:"bold"}}>
                {h.count}<span style={{color:T.textMuted,fontWeight:"normal",fontSize:11}}> / {h.capacity}</span>
              </div>
            </div>
            <div style={{height:6,background:"rgba(0,0,0,0.3)",borderRadius:6,overflow:"hidden",
              boxShadow:"inset 0 1px 3px rgba(0,0,0,0.4)"}}>
              <div style={{height:"100%",width:`${Math.min((h.count/h.capacity)*100,100)}%`,
                background:`linear-gradient(90deg,#8B6914,#C49A28 50%,#e8c84a)`,
                borderRadius:6,boxShadow:`0 0 14px rgba(196,154,40,0.6)`}}/>
            </div>
          </div>
        </div>
        <div style={{borderTop:`1px solid rgba(160,120,40,0.10)`,padding:"11px 22px",
          display:"flex",alignItems:"center",justifyContent:"space-between",background:"rgba(0,0,0,0.18)"}}>
          <div style={{fontSize:11,color:T.textMuted,fontFamily:"Georgia,serif",fontStyle:"italic"}}>
            {status==="connected"?`Updated ${lastUpdated}`:"Connect Govee H5179 for live readings"}
          </div>
          {isHero&&status==="connected"&&(
            <button onClick={fetchLive} style={{background:"none",border:"none",color:T.goldMid,
              fontSize:11,cursor:"pointer",fontFamily:"Georgia,serif",padding:0}}>Refresh</button>
          )}
        </div>
      </div>
    );
  ...
  return (
    <div style={{padding:"0 0 36px"}}>
      ...
      {/* Add Humidor Form */}
      {showForm && (
        <div style={{margin:"14px 16px 0",background:"linear-gradient(170deg,#2c1a08,#1e0e04)",
  ...
  return (
    <div style={{padding:"0 0 32px"}}>
      ...
      <div style={{padding:"24px 20px 20px",borderBottom:`1px solid ${T.border}`}}>
        <div style={{fontSize:10,letterSpacing:5,textTransform:"uppercase",color:T.textMuted,fontFamily:"Georgia,serif",marginBottom:14}}>My Collection</div>
        <div style={{display:"flex",gap:28,alignItems:"baseline"}}>
          <div>
            <div style={{fontSize:36,fontWeight:"bold",color:T.textPrimary,fontFamily:"Georgia,serif",lineHeight:1}}>{total}</div>
            <div style={{fontSize:10,color:T.textMuted,letterSpacing:2,textTransform:"uppercase",marginTop:5}}>Total Cigars</div>
          </div>
          <div style={{width:1,height:36,background:T.border}}/>
          <div>
            <div style={{fontSize:36,fontWeight:"bold",color:T.textPrimary,fontFamily:"Georgia,serif",lineHeight:1}}>94.7</div>
            <div style={{fontSize:10,color:T.textMuted,letterSpacing:2,textTransform:"uppercase",marginTop:5}}>Avg Rating</div>
          </div>
        </div>
      </div>

      {/* Scanner */}
      <div style={{margin:"16px 16px 0",background:T.card,borderRadius:12,border:`1px solid ${T.border}`,
        padding:"14px 16px",display:"flex",alignItems:"center",gap:14,cursor:"pointer"}}>
        <div style={{width:36,height:36,borderRadius:8,background:`${T.blueMid}33`,border:`1px solid ${T.blueMid}44`,
          display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>🔍</div>
        <div style={{flex:1}}>
          <div style={{fontSize:14,fontWeight:"bold",color:T.textPrimary,fontFamily:"Georgia,serif",marginBottom:2}}>Band Scanner</div>
          <div style={{fontSize:11,color:T.textMuted}}>AI identification from cigar band photo</div>
        </div>
      ...
      <div style={{padding:"20px 20px 12px",display:"flex",justifyContent:"space-between",alignItems:"baseline"}}>
        <div style={{fontSize:10,letterSpacing:4,textTransform:"uppercase",color:T.textMuted,fontFamily:"Georgia,serif"}}>Cigars</div>
        <div style={{fontSize:11,color:T.textGold,cursor:"pointer",fontFamily:"Georgia,serif"}}>Sort: Recently Added</div>
      </div>

      {/* CIGAR BAND CARDS — premium catalog */}
      <div style={{padding:"0 16px"}}>
        {CIGARS.map(c=>{
          const isEx=sel===c.id;
          return (
            <div key={c.id} onClick={()=>setSel(isEx?null:c.id)}
              style={{background:T.card,borderRadius:14,marginBottom:14,overflow:"hidden",
              ...
              <div style={{display:"flex",alignItems:"stretch",minHeight:96}}>
                ...
                <div style={{flex:1,padding:"18px 16px"}}>
                  ...
                  {isEx && (
                    <div style={{marginTop:14,paddingTop:14,borderTop:`1px solid ${T.border}`,display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        ...
        <button style={{width:"100%",padding:"13px",background:"none",border:`1px dashed ${T.border}`,
          borderRadius:12,color:T.textMuted,fontSize:11,fontFamily:"Georgia,serif",
...
const QUICK_PROMPTS=["Recommend Cigars","Humidor Advice","Pairing Suggestion","What to smoke tonight?"];

function AskMarioTab() {
  const [messages,setMessages]=useState([{role:"ai",text:"Good evening, Zebulon.\n\nYour Padrón 1964 has been resting 31 months at 69% RH.\n\nTonight feels like the right moment.\n\n— Mario"}]);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 130px)"}}>
      ...
      {/* MARIO HERO — large, welcoming, concierge feel */}
      <div style={{flexShrink:0,background:`linear-gradient(175deg,#0c1828 0%,#080e18 100%)`,
        padding:"22px 20px 18px",borderBottom:`1px solid ${T.blueMid}33`}}>
        <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:16}}>
          <div style={{width:60,height:60,borderRadius:"50%",flexShrink:0,
            background:`linear-gradient(135deg,${T.goldDark},${T.goldMid})`,
            display:"flex",alignItems:"center",justifyContent:"center",
            fontSize:26,fontWeight:"bold",color:"#1a0e04",fontFamily:"Georgia,serif",
            boxShadow:`0 0 0 3px #0c1828,0 0 0 5px ${T.goldDark}33,0 8px 28px rgba(0,0,0,0.5)`}}>M</div>
          <div style={{flex:1}}>
            <div style={{fontSize:22,fontWeight:"bold",color:"#e8dcc8",fontFamily:"Georgia,serif",marginBottom:3}}>Ask Mario</div>
            <div style={{fontSize:10,color:"#3a5070",letterSpacing:2.5,textTransform:"uppercase"}}>Master Cigar Sommelier · Private Lounge</div>
          </div>
        </div>

        {/* Tonight's Recommendation card */}
        <div style={{background:"linear-gradient(155deg,#fdfaf2 0%,#f5e8cc 55%,#eddfc0 100%)",
          borderRadius:14,padding:"18px 20px",
          boxShadow:"0 8px 32px rgba(0,0,0,0.4),inset 0 1px 0 rgba(255,255,255,0.7)"}}>
          <div style={{fontSize:9,color:"#8B6914",letterSpacing:3,textTransform:"uppercase",marginBottom:10,fontFamily:"Georgia,serif"}}>Tonight's Recommendation</div>
          <div style={{fontSize:20,fontWeight:"bold",color:"#2a1608",fontFamily:"Georgia,serif",marginBottom:4}}>Padrón 1964 Exclusivo</div>
          <div style={{display:"flex",gap:16,marginBottom:12}}>
            <div style={{fontSize:12,color:"#6a4020"}}>Rested 31 months</div>
            <div style={{fontSize:12,color:"#6a4020"}}>69% RH</div>
          </div>
          <div style={{borderTop:"1px solid rgba(139,105,20,0.2)",paddingTop:10}}>
            <div style={{fontSize:9,color:"#8B6914",letterSpacing:2,textTransform:"uppercase",marginBottom:6}}>Pair with</div>
            <div style={{display:"flex",gap:8,flexWrap:"wrap" as const}}>
              {["Espresso","Blanton's Bourbon","Dark Chocolate"].map(p=>(
                <span key={p} style={{fontSize:11,color:"#4a2810",background:"rgba(139,105,20,0.08)",
                  border:"1px solid rgba(139,105,20,0.2)",borderRadius:20,padding:"3px 10px",fontFamily:"Georgia,serif"}}>{p}</span>
              ))}
            </div>
          </div>
        </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"16px 16px 0",display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
            {m.role==="ai" ? (
              <div style={{maxWidth:"92%",
                background:"linear-gradient(155deg,#fdfaf2 0%,#f5e8cc 55%,#eddfc0 100%)",
                border:`1px solid rgba(160,120,50,0.22)`,borderRadius:"3px 16px 16px 16px",
                padding:"18px 20px",boxShadow:"0 4px 20px rgba(0,0,0,0.3),inset 0 1px 0 rgba(255,255,255,0.6)"}}>
                <div style={{fontSize:9,color:"#8B6914",letterSpacing:2,textTransform:"uppercase",
                  marginBottom:10,display:"flex",alignItems:"center",gap:6}}>
                  <div style={{width:14,height:14,borderRadius:"50%",background:`linear-gradient(135deg,${T.goldDark},${T.goldMid})`,
                    display:"flex",alignItems:"center",justifyContent:"center",fontSize:7,fontWeight:"bold",color:"#1a0e04"}}>M</div>
                  Mario
                </div>
                <div style={{fontSize:14,color:"#2a1608",lineHeight:1.85,fontFamily:"Georgia,serif",fontStyle:"italic",whiteSpace:"pre-line"}}>{m.text}</div>
              </div>
            ) : (
              <div style={{maxWidth:"80%",background:`${T.blueMid}28`,border:`1px solid ${T.blueMid}33`,
        ...
        {loading && (
          <div style={{display:"flex",justifyContent:"flex-start"}}>
            <div style={{background:"linear-gradient(155deg,#fdfaf2,#f5e8cc)",border:`1px solid rgba(160,120,50,0.22)`,
              borderRadius:"3px 16px 16px 16px",padding:"16px 20px"}}>
              <div style={{display:"flex",gap:5}}>{[0,1,2].map(i=><div key={i} style={{width:6,height:6,borderRadius:"50%",background:T.goldDark,animation:`mT 1.2s ease-in-out ${i*0.2}s infinite`}}/>)}</div>
            </div>
          </div>
        )}
        <div ref={bottomRef}/>
      ...
      {/* Quick prompts */}
      <div style={{padding:"10px 16px 6px",display:"flex",flexWrap:"wrap" as const,gap:8,flexShrink:0}}>
        {QUICK_PROMPTS.map(q=>(
          <button key={q} onClick={()=>send(q)}
            style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:20,
              padding:"7px 16px",color:T.textSecondary,fontSize:12,cursor:"pointer",fontFamily:"Georgia,serif"}}>
            {q}
          </button>
        ))}
      </div>
      ...
      {/* Input */}
      <div style={{padding:"4px 16px 16px",display:"flex",gap:10,alignItems:"center",flexShrink:0}}>
        ...
        <button onClick={()=>send(input)}
          style={{width:44,height:44,borderRadius:"50%",flexShrink:0,
            ...
            border:"none",cursor:"pointer",color:"#1a0e04",fontSize:22,fontWeight:"bold",
...
const TRENDING=[
  {title:"Tonight's Smoke",sub:"What are you enjoying tonight?",comments:24,time:"2m ago"},
  {title:"Humidor Setups",sub:"Let's see your configurations.",comments:18,time:"15m ago"},
  {title:"Cigar Reviews",sub:"Recent ratings from the community.",comments:31,time:"1h ago"},
];
...
function CommunityTab() {
  ...
  const submitPost=()=>{
    ...
    setPosts(p=>[{id:Date.now(),user:"You",avatar:"Y",badge:"",category:newPost.category,
      time:"Just now",title:newPost.title,body:newPost.body,likes:0,comments:0,liked:false},...p]);
    setNewPost({title:"",body:"",category:"Review"});
    setShowNew(false);
  };
  const toggleLike=(id:number)=>setPosts(p=>p.map(x=>x.id===id?{...x,liked:!x.liked,likes:x.liked?x.likes-1:x.likes+1}:x));
  ...
  return (
    <div style={{padding:"0 0 32px"}}>
      ...
      <div style={{padding:"24px 20px 20px",borderBottom:`1px solid ${T.border}`,
        ...
        <div>
          <div style={{fontSize:10,letterSpacing:5,textTransform:"uppercase",color:T.textMuted,fontFamily:"Georgia,serif",marginBottom:6}}>Community</div>
          <div style={{fontSize:22,fontWeight:"bold",color:T.textPrimary,fontFamily:"Georgia,serif"}}>The Members Lounge</div>
        </div>
      ...
      {/* New post form */}
      {showNew && (
        <div style={{margin:"16px 16px 0",background:T.card,borderRadius:12,border:`1px solid ${T.borderGold}`,padding:16}}>
          <select value={newPost.category} onChange={e=>setNewPost(p=>({...p,category:e.target.value}))} style={{...fi}}>
            {["Review","Pairing","Question","Storage","Event"].map(c=><option key={c}>{c}</option>)}
          </select>
          <input value={newPost.title} onChange={e=>setNewPost(p=>({...p,title:e.target.value}))} placeholder="Title..." style={fi}/>
          <textarea value={newPost.body} onChange={e=>setNewPost(p=>({...p,body:e.target.value}))} placeholder="Share your experience..." rows={3} style={{...fi,resize:"none"}}/>
          <div style={{display:"flex",gap:10}}>
            <button onClick={submitPost} style={{flex:1,padding:"10px",background:`linear-gradient(135deg,${T.goldDark},${T.goldMid})`,border:"none",borderRadius:8,color:"#1a0e04",fontSize:13,fontWeight:"bold",cursor:"pointer",fontFamily:"Georgia,serif"}}>Post</button>
            <button onClick={()=>setShowNew(false)} style={{padding:"10px 16px",background:"transparent",border:`1px solid ${T.border}`,borderRadius:8,color:T.textMuted,fontSize:13,cursor:"pointer"}}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div style={{padding:"20px 20px 0"}}>
        <div style={{fontSize:10,letterSpacing:4,textTransform:"uppercase",color:T.textMuted,fontFamily:"Georgia,serif",marginBottom:12}}>Trending</div>
        <div style={{display:"flex",flexDirection:"column",gap:0,background:T.card,borderRadius:12,border:`1px solid ${T.border}`,overflow:"hidden"}}>
          {TRENDING.map((t,i)=>(
            <div key={i} style={{padding:"14px 16px",borderBottom:i<TRENDING.length-1?`1px solid ${T.border}`:"none",
              display:"flex",alignItems:"center",gap:12,cursor:"pointer"}}>
              <div style={{width:36,height:36,borderRadius:8,background:`${T.goldDark}18`,
                border:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"center",
                fontSize:14,flexShrink:0}}>💬</div>
              <div style={{flex:1}}>
                <div style={{fontSize:14,fontWeight:"bold",color:T.textPrimary,fontFamily:"Georgia,serif",marginBottom:2}}>{t.title}</div>
                <div style={{fontSize:11,color:T.textMuted}}>{t.sub}</div>
              </div>
              <div style={{textAlign:"right",flexShrink:0}}>
                <div style={{fontSize:12,color:T.textSecondary,marginBottom:1}}>{t.comments} replies</div>
                <div style={{fontSize:10,color:T.textMuted}}>{t.time}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Filter tabs */}
      <div style={{padding:"0 20px",borderBottom:`1px solid ${T.border}`,display:"flex",gap:0,marginTop:20}}>
        {["All","Review","Pairing","Question"].map(f=>(
          <button key={f} onClick={()=>setFilter(f)}
            style={{padding:"12px 0",marginRight:24,background:"none",border:"none",
              borderBottom:filter===f?`2px solid ${T.goldMid}`:"2px solid transparent",
              color:filter===f?T.textPrimary:T.textMuted,fontSize:13,cursor:"pointer",
              fontFamily:"Georgia,serif",fontWeight:filter===f?"bold":"normal",letterSpacing:0.3}}>
            {f}
          </button>
        ))}
      </div>
      ...
      <div style={{padding:"16px 16px 0"}}>
        {filtered.map(post=>{
          const cc=CAT_COLORS[post.category]||T.textMuted;
          return (
            <div key={post.id} style={{background:T.card,borderRadius:14,border:`1px solid ${T.border}`,marginBottom:14,overflow:"hidden"}}>
              <div style={{padding:"16px 18px"}}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                  <div style={{width:36,height:36,borderRadius:"50%",background:`${cc}18`,border:`1px solid ${cc}28`,
                    display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:"bold",color:cc,flexShrink:0}}>
                    {post.avatar}
                  </div>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <div style={{fontSize:13,fontWeight:"bold",color:T.textPrimary}}>{post.user}</div>
                      {post.badge && <span style={{fontSize:9,color:T.textGold,background:`${T.goldDark}18`,border:`1px solid ${T.goldDark}28`,borderRadius:20,padding:"1px 8px",letterSpacing:0.5}}>{post.badge}</span>}
                    </div>
                    <div style={{fontSize:10,color:T.textMuted,marginTop:2}}>{post.time}</div>
                  </div>
                  <div style={{background:`${cc}12`,border:`1px solid ${cc}25`,borderRadius:20,padding:"2px 10px",fontSize:9,color:cc,letterSpacing:1}}>{post.category}</div>
                </div>
                <div style={{fontSize:16,fontWeight:"bold",color:T.textPrimary,fontFamily:"Georgia,serif",lineHeight:1.3,marginBottom:8}}>{post.title}</div>
                <div style={{fontSize:13,color:T.textSecondary,lineHeight:1.75,marginBottom:14}}>{post.body}</div>
                <div style={{display:"flex",gap:20,paddingTop:12,borderTop:`1px solid ${T.border}`}}>
                  <button onClick={()=>toggleLike(post.id)}
                    style={{background:"none",border:"none",cursor:"pointer",color:post.liked?T.goldLight:T.textMuted,
                      fontSize:13,fontFamily:"Georgia,serif",display:"flex",alignItems:"center",gap:5}}>
                    {post.liked?"♥":"♡"} {post.likes}
                  </button>
                  <span style={{color:T.textMuted,fontSize:13,display:"flex",alignItems:"center",gap:5}}>💬 {post.comments}</span>
                </div>
              </div>
            </div>
          );
      ...
      <div style={{padding:"16px 16px 0"}}>
        {notes.map(n=>(
          <div key={n.id} onClick={()=>setSel(sel===n.id?null:n.id)}
            style={{background:T.card,borderRadius:14,border:`1px solid ${sel===n.id?T.borderGold:T.border}`,
              marginBottom:16,overflow:"hidden",cursor:"pointer"}}>
            {/* Parchment header */}
            <div style={{background:"linear-gradient(160deg,#fdfaf2 0%,#f0e4c8 100%)",padding:"16px 18px 14px"}}>
              {/* Date */}
              <div style={{fontSize:10,color:"#8B6914",letterSpacing:3,textTransform:"uppercase",marginBottom:8,fontFamily:"Georgia,serif"}}>{n.date}</div>
              {/* Cigar name */}
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div style={{flex:1,paddingRight:12}}>
                  <div style={{fontSize:10,color:"#7a5030",letterSpacing:3,textTransform:"uppercase",marginBottom:4,fontFamily:"Georgia,serif"}}>{n.brand}</div>
                  <div style={{fontSize:20,fontWeight:"bold",color:"#2a1608",fontFamily:"Georgia,serif",marginBottom:3}}>{n.line}</div>
                  <div style={{fontSize:12,color:"#6a4020"}}>{n.vitola}</div>
                </div>
                {/* Star rating */}
                <div style={{display:"flex",gap:1,flexShrink:0}}>
                  {[1,2,3,4,5].map(i=><span key={i} style={{fontSize:16,color:i<=n.rating?"#C49A28":"rgba(139,105,20,0.2)",lineHeight:1}}>★</span>)}
                </div>
              </div>
            </div>
            {/* Notes body */}
            {sel===n.id && (
              <div style={{padding:"16px 18px",borderTop:"1px solid rgba(139,105,20,0.15)"}}>
                <div style={{fontSize:14,color:T.textSecondary,fontFamily:"Georgia,serif",
                  fontStyle:"italic",lineHeight:1.85,marginBottom:14}}>{n.notes}</div>
                {n.pairing && (
                  <div style={{display:"flex",alignItems:"center",gap:8,
                    background:"rgba(0,0,0,0.15)",borderRadius:8,padding:"9px 13px"}}>
                    <span>🥃</span>
                    <span style={{fontSize:13,color:T.textGold,fontFamily:"Georgia,serif"}}>Paired with {n.pairing}</span>
                  </div>
                )}
              </div>
            )}
        ...
        {NEWS.map(n=>(
          <div key={n.id} style={{background:T.card,borderRadius:16,border:`1px solid ${T.border}`,overflow:"hidden"}}>
            ...
            <div style={{padding:"18px 18px 16px"}}>
              ...
              <div style={{fontSize:15,fontWeight:"bold",color:T.textPrimary,fontFamily:"Georgia,serif",lineHeight:1.35,marginBottom:8}}>{n.title}</div>
              <div style={{fontSize:12,color:T.textSecondary,lineHeight:1.6}}>{n.summary}</div>
...
function BottomNav({tab,setTab}:{tab:string,setTab:(t:string)=>void}) {
  return (
    <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,zIndex:100,background:T.cardMid,borderTop:`1px solid ${T.border}`}}>
      <CubanRibbon height={2}/>
...
function AppHeader({totalCigars}:{totalCigars:number}) {
  return (
    <div style={{position:"sticky",top:0,zIndex:50,background:"#1a0e04",borderBottom:`1px solid rgba(160,120,40,0.2)`}}>
      <CubanRibbon height={3}/>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"11px 18px 12px"}}>
        {/* Left — medallion + title */}
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <div style={{width:36,height:36,borderRadius:"50%",flexShrink:0,
            background:`linear-gradient(135deg,${T.goldDark},${T.goldMid})`,
            display:"flex",alignItems:"center",justifyContent:"center",
            fontSize:16,fontWeight:"bold",color:"#1a0e04",fontFamily:"Georgia,serif",
            boxShadow:`0 0 0 2px #1a0e04, 0 0 0 3px ${T.goldDark}66, 0 4px 14px rgba(0,0,0,0.5)`}}>M</div>
          <div>
            <div style={{fontSize:16,fontWeight:"bold",color:T.textPrimary,fontFamily:"Georgia,serif",letterSpacing:0.2,lineHeight:1.15}}>Mario's Humidor</div>
            <div style={{fontSize:9,color:T.textMuted,letterSpacing:2,textTransform:"uppercase",marginTop:2}}>The Cigar Lifestyle Platform</div>
          </div>
        </div>
        {/* Right — cigar count + bell */}
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:20,fontWeight:"bold",color:T.goldLight,fontFamily:"Georgia,serif",lineHeight:1}}>{totalCigars}</div>
            <div style={{fontSize:8,color:T.textMuted,letterSpacing:1.5,textTransform:"uppercase",marginTop:1}}>Cigars</div>
          </div>
          {/* Notification bell */}
          <div style={{width:32,height:32,borderRadius:"50%",background:"rgba(255,255,255,0.06)",
            display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" stroke={T.textSecondary} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M13.73 21a2 2 0 0 1-3.46 0" stroke={T.textSecondary} strokeWidth="1.6" strokeLinecap="round"/>
            </svg>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:1000,
      ...
      <div style={{position:"relative",zIndex:1,textAlign:"center",padding:"0 40px"}}>
        <div style={{width:88,height:88,borderRadius:"50%",margin:"0 auto 24px",
          background:`linear-gradient(135deg,${T.goldDark},${T.goldMid})`,
          display:"flex",alignItems:"center",justifyContent:"center",
          fontSize:44,fontWeight:"bold",color:"#1a0e04",fontFamily:"Georgia,serif",
          boxShadow:`0 0 0 6px #120a02,0 0 0 8px ${T.goldDark}33,0 12px 40px ${T.goldDark}44`}}>M</div>
        <div style={{fontSize:24,fontWeight:"bold",color:T.textPrimary,fontFamily:"Georgia,serif",letterSpacing:1,marginBottom:10}}>Mario's Humidor</div>
        <CubanRibbon height={2}/>
        <div style={{fontSize:10,color:T.textMuted,letterSpacing:4,textTransform:"uppercase",marginTop:10,marginBottom:36}}>
          The Cigar Lifestyle Platform
        </div>
        <div style={{display:"flex",gap:7,justifyContent:"center"}}>
          {[0,1,2].map(i=><div key={i} style={{width:5,height:5,borderRadius:"50%",background:T.goldDark,
            animation:`sp 1.4s ease-in-out ${i*0.28}s infinite`}}/>)}
        </div>
      </div>
      <style>{`@keyframes sp{0%,100%{opacity:0.2;transform:scale(0.7)}50%{opacity:1;transform:scale(1.2)}}`}</style>
    </div>
  );
}
...
export default function MariosHumidor() {
  ...
  const totalCigars=CIGARS.reduce((a,c)=>a+c.count,0);
  const render=()=>{
    switch(tab){
      case "humidors":   return <HumidorsTab/>;
      case "collection": return <CollectionTab/>;
      case "mario":      return <AskMarioTab/>;
  ...
  return (
    <div style={{minHeight:"100vh",background:T.bg,color:T.textPrimary,fontFamily:"Georgia,serif",maxWidth:480,margin:"0 auto",position:"relative"}}>
      ...
      <div style={{position:"relative",zIndex:1,paddingBottom:76}}>
        <AppHeader totalCigars={totalCigars}/>
        {render()}
      </div>
